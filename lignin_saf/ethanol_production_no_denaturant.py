# -*- coding: utf-8 -*-
"""
Variant of ethanol_production.py for use when the ethanol product is fed
to catalytic upgrading (e.g. the ETJ system) rather than sold as fuel-grade
ethanol.  Denaturant (octane) is not added: M701.denaturant_fraction is set
to 0.0 so the adjust_denaturant spec resolves to zero flow every iteration.
All other unit operations, stream routing, and facility handling are
identical to ethanol_production.py.
"""
...
def create_cellulosic_ethanol_system(ins, outs):
    ...
    # Zero out the denaturant — the adjust_denaturant spec on M701 will keep
    # it at zero every iteration, so no octane enters the ethanol product.
