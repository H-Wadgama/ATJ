...
_What would be needed to fully capture 3-hour solvent contact time_: To model accumulated contact time correctly, a recirculation pump and loop internal to the solvolysis step would need to be added (bleed ≈ 250 m³/hr net to hydrogenolysis; recirculation ≈ 2,208 m³/hr back to reactor inlet per operating bed). This would add pump CAPEX and OPEX but would not change the mass balance or conversion since conversion is kinetically fixed in the literature data used.

_Solvolysis reactor geometry_: Each bed is sized as D ≈ 6.3 m, L ≈ 20 m (L/D ≈ 3.1) based on a superficial velocity of 0.01 m/s and a flow rate Q = V_solvent / tau_residence = 368 m³ / (1/3 hr) ≈ 1,104 m³/hr per reactor (total across 3 operating beds: 3,312 m³/hr). Note that the hydraulic residence time is defined on the actual solvent volume in the bed (368 m³), not the total vessel volume (600 m³). The vessel cost is extrapolated outside BioSTEAM's built-in correlation range (which covers L ≤ 40 ft / 12 m) — this is expected for large custom pressure vessels of this scale.
