...
class bleh(bst.Unit):
    ...
    def __init__(self, ID="", ins=None, outs=(), thermo=None, uptime_ratio = 0.9, conversion = 0.9, temperature = 393.15, pressure = 3e+6, WHSV = 1.5, 
    ...
    def _run(self):
        fresh_biomass, = self.ins
        used_biomass, = self.outs
        used_biomass.mix_from(self.ins)
        self.reaction(used_biomass)
        used_biomass.P = fresh_biomass.P
        # used_solvent.imass['Methanol'] = fresh_solvent.imass['Methanol']*self.conversion
    ...
    def _cost(self):
        ...
        C_v = 2.1*(math.exp(5.6336 + 0.4599 * lnW + 0.00582 * lnW * lnW)) # 2.1 as vessel will be SS312 
